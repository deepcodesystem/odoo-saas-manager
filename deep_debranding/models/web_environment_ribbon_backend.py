...
class WebEnvironmentRibbonBackend(models.AbstractModel):
    ...
    def get_environment_ribbon(self):
        """Retourne les données du ruban depuis ir.config_parameter."""
        config = self.env["ir.config_parameter"].sudo()
        name = self._prepare_ribbon_name()
        return {
            "name": name,
            "color": config.get_param("app_ribbon_color"),
            "background_color": config.get_param("app_ribbon_background_color"),
        }
